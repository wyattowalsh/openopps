...
- Jobs/Explorer search artifacts live in `web/public/data/openopps-search/` and are generated by `scripts/generate_docs_search_index.py` from ignored local `kaggle/openoppsdb.sqlite`. Refresh with `just web-search-index`. Tiered detail shards use T1 metadata-only and T2 bounded plain-text bodies (≤4000 chars); raw `payloadSnapshots` stay in SQLite/Kaggle only (surplus S1–S4 taxonomy in `openspec/changes/ingest-data-surplus/`).
- Docs search artifact gates: CI (docs job / `just web-check`) runs `uv run pytest tests/unit/openopps/test_docs_search_index.py -k committed` plus `scripts/verify_docs_search_artifacts.py --require-git-tracked` on the committed tree—no local SQLite and no regeneration. `just web-search-artifacts-check` is the local equivalent of that verifier. `just web-search-index-check` is maintainer-only before release: requires `kaggle/openoppsdb.sqlite`, regenerates the snapshot, then fails on remaining git drift; CI does not run it.
- Dependency updates: Renovate (`renovate.json`) owns PEP 621 / `uv.lock` and docs npm (`web/package.json` / `pnpm-lock.yaml`). Dependabot (`.github/dependabot.yml`) owns `github-actions` pin bumps. Prefer Renovate for web npm; treat Renovate as canonical and close or ignore duplicate Dependabot `/web` npm PRs to avoid overlapping lock churn.
