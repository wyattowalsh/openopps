...
- Docs Jobs/Explorer search artifacts live in `docs/public/data/openopps-search/` and are generated by `scripts/generate_docs_search_index.py` from ignored local `kaggle/openoppsdb.sqlite`. Refresh with `just docs-search-index`; release parity with `just docs-search-index-check`; committed graph completeness with `just docs-search-artifacts-check`. CI docs job runs `pytest -k committed` on `test_docs_search_index.py` for manifest schema checks. Tiered detail shards use T1 metadata-only and T2 bounded plain-text bodies (≤4000 chars); raw `payloadSnapshots` stay in SQLite/Kaggle only (surplus S1–S4 taxonomy in `openspec/changes/ingest-data-surplus/`).
