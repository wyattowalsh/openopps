...
- Dependency updates: Renovate (`renovate.json`) owns PEP 621 / `uv.lock` and web npm (`web/package.json` / `pnpm-lock.yaml`). Dependabot (`.github/dependabot.yml`) owns `github-actions` pin bumps.
...
- Keep OpenOpps CLI-first. Do not add prompt, TUI, browser, web app, or hosted-service flows unless the user explicitly changes product scope. URL pull does not populate the catalog; packaged overlay JSON is maintainer-owned; `jobs pull --metrics-file` is the pull observability channel (`--metrics-json` remains catalog sync stdout only). `jobs export` and `jobs pull` accept `--profile core|search|full|raw` (default `full`); web search artifacts stay on `search`. Do not attach coverage-quality JSON to `--metrics-json` or `--metrics-file`.
- Live authority remains separate from local discovery work: no Cloudflare Workers upload, no Kaggle mutation, and source-policy 1780 stays blocked pending written grants. Alembic `0005_update_snapshot_ledger` then `0006_url_pull_runs` have landed (live `uv run alembic heads` is `0006_url_pull_runs` only). URL-pull persistence is opt-in `--save` (default False; `--no-save` ephemeral; persist failure exit 9). Unscoped `jobs sync` excludes reserved `url-pull` routes. Do not treat that closeout as Workers, Kaggle, GitHub Release, PyPI, hosted-alpha, or v7 7.6 proof. Offline scout/verify/preview and `just ci-discovery` are not live publication proof.
