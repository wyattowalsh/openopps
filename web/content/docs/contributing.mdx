...
`just ci` runs `diff-check`, `lock-check`, `openspec-validate-all`, `test-cov`, `web-check`, `web-build`, `web-test`, `web-e2e`, `web-a11y`, `web-lint`, `kaggle-meta`, and `cli-help`. `just web-rtk-lint` is the explicit optional maintainer lint for `rtk`; it is not part of the default CI recipe.
...
CI never opens SQLite. It validates the **committed** search artifact graph with `just web-search-artifacts-check` and the schema check inside `just web-check` (`pytest -k committed`). Run `just web-search-index-check` before release only when intentionally regenerating `web/public/data/openopps-search/` from a clean local snapshot.
