...
| `OPENOPPS_CONCURRENCY_PROFILE` | `explicit` | `explicit` keeps the integer knobs on this page. `auto` / `ci` / `constrained` may size JSON-board source, board, and provider pools from available CPUs, but they never raise Workday or discovery per-host caps. Explicit `OPENOPPS_*` integers always win. |
| `OPENOPPS_WORKDAY_CONCURRENCY`  | `2`                                                       | Conservative limit for public Workday CXS careers-site requests. Profiles never raise this value. |
...
The isolated scout uses a separate settings model (`DiscoverySettings` in `src/openopps/discovery/settings.py`). `OPENOPPS_DISCOVERY_*` budget fields are not part of `OpenOppsSettings`, do not load `.env`, and do not change `openopps sync`. `OPENOPPS_CONCURRENCY_PROFILE` never derives `OPENOPPS_DISCOVERY_PER_HOST_CONCURRENCY`. Values must be canonical positive decimals. Invalid values fail closed without echoing the input.
...
OPENOPPS_CONCURRENCY_PROFILE=explicit
...
`DiscoverySettings` ignores `.env`; do not put `OPENOPPS_DISCOVERY_*` or `OPENOPPS_DISCOVERY_NETWORK` in that file. Unscoped `jobs sync` targets persisted ready catalog routes and excludes reserved `url-pull` identity. Use explicit CLI filters such as `--source`, `--board`, or `--provider` for one-off scoped syncs. Do not pin `--source url-pull`.

URL pull does not use an environment persist switch. `jobs pull` and `openopps <URL>` stay ephemeral unless you pass `--save`; `--no-save` is the explicit alias for that default. HTTP cache writes remain independently governed. Persist failure exits `9` (`PERSISTENCE_FAILED`) and does not claim a saved success. See [CLI](/docs/cli#url-pull) and [Operations](/docs/operations).
...
| Persist one URL pull                         | `--save` (opt-in; default False)        |
| Keep one URL pull ephemeral                   | `--no-save` (default; persist failure exits 9) |
| Always bypass cache reads in a shell/session | `OPENOPPS_CACHE_REFRESH=true`           |
| Narrow one job sync                          | `--source`, `--board`, or `--provider`  |
| Opt into place-aware JSON-board pools        | `OPENOPPS_CONCURRENCY_PROFILE=auto` (Workday stays 2) |
