...
### Docs app public data origin

These variables configure the Fumadocs/Next.js docs host when it loads committed search artifacts server-side. They are **not** part of the CLI `OpenOppsSettings` model.

| Variable | Default | Purpose |
| -------- | ------- | ------- |
| `OPENOPPS_PUBLIC_DATA_ORIGIN` | site URL | Origin used for server-side fetches of `/data/openopps-search/*` (never the request Host). Must be `https://` in production unless insecure override is set. |
...
Local `next start` / Playwright e2e prefer loopback `http://127.0.0.1:<port>/` and load shards from `docs/public` on disk so server-side self-HTTP is not required.
