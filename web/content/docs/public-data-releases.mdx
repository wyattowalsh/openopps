...
Run `just source-policy-check` for the structural CI contract and `just source-policy-audit` for release eligibility. The current committed-v6 policy is structurally valid but release-ineligible: 7 of 695 sources only mirror repository catalog declarations, 0 are independently verified, and 688 are blocked. The audit exits 2, so do not render a selector, generate or upload a production corpus, bootstrap a Worker, or publish it.
