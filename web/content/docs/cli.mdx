...
`status` and `doctor` sit on the Everyday workflow panel. `doctor` prints the same JSON-shaped status payload as `status`, then a first-time checklist: set `OPENOPPS_DB_URL`, run `openopps admin db init` (idempotent), then fill the catalog with `openopps sync a16z --metrics-json` or `examples seed`. `openopps https://…` / `jobs pull` is an ephemeral jobs fetch unless `--save` is passed; it does not populate catalog sources. Successful `doctor` does not claim the schema is missing. `nextAction` never names `discovery scout`.

The normal sequence is:

1. Fill the catalog with `openopps sync a16z --metrics-json` or `examples seed`. Optionally pull a public URL with `openopps https://…` (ephemeral unless `--save`).
2. Use `sources sync`, `boards sync`, or `jobs sync` when rerunning one stage. Unscoped `jobs sync` excludes reserved `url-pull` routes.
...
| Writes local SQLite state | `examples seed`, `sources sync`, `boards sync`, `jobs sync`, `jobs pull --save`, `admin boards add`, `admin db init` | Creates or updates records under `OPENOPPS_DB_URL` and cache settings. Default `jobs pull` is ephemeral. |
...
`--apply` is intentionally absent from most everyday commands. When it appears on diagnostics such as route probing or health checks, treat it as the boundary between inspection and persistence. URL pull uses `--save` / `--no-save` (default False), not `--apply`. Quarantined discovery commands never expose `--apply`.
...
| `jobs` | `pull`, `sync`, `list`, `show`, `history`, `export` | URL-first pull (opt-in `--save`), plus catalog sync, inspect, version, and export. |
...
| `--profile` | `jobs export` and `jobs pull` only: `core`, `search`, `full`, or `raw`. Default `full`. Does not change `jobs sync`, `jobs list`, `jobs show`, or `jobs history`. |
...
JSON/JSONL job objects identify `profile` and `schemaVersion`. CLI export and pull default to `full`. Docs Jobs/Explorer search artifacts use the `search` profile. `core`/`search`/`full`/`raw` do not authorize Workers upload, Kaggle mutation, or a live hosted database.
...
`openopps <URL>` and `openopps jobs pull <URL>` share one service. They do not populate the source catalog.

```bash
uv run openopps jobs pull https://jobs.ashbyhq.com/example --json
uv run openopps https://jobs.ashbyhq.com/example --no-save --metrics-file /tmp/pull-metrics.json
uv run openopps jobs pull https://jobs.ashbyhq.com/example --save
```

| Rule | Meaning |
| ---- | ------- |
| Identity | Reserved source key `url-pull` plus a punctuation-preserving digest. Not a catalog source. |
| `--save` | Opt-in. CLI default is False. Writes a complete validated result only. |
| `--no-save` | Explicit ephemeral alias for the default. Operational tables stay unchanged. |
| HTTP cache | Independent of save/no-save. `--refresh-cache` controls freshness only. |
| Complete-before-apply | Membership and authority must validate before any ledger write. |
| Persist failure | Exit 9 (`PERSISTENCE_FAILED`). Does not claim a saved success. |
| Unscoped `jobs sync` | Excludes reserved `url-pull` routes. |
| Observability | `--metrics-file` or `--raw`. Never `--metrics-json` (catalog sync stdout only). |

`--profile core|search|full|raw` (default `full`) applies to pull JSON the same way it does to `jobs export`.
