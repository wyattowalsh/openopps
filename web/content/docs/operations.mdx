...
`admin db init` applies the current v0.1 Alembic schema. Live `uv run alembic heads` is `0006_url_pull_runs` only. `0005_update_snapshot_ledger` is the landed update-snapshot ledger; `0006` adds `url_pull_runs` and membership-scoped URL-pull persistence. `0005` is no longer blocked. HTTP response cache rows live in that same SQLite database and are managed by `cache.py`, not by the Alembic schema history. Unsupported or partial pre-release schemas fail closed; reset that local database or point `OPENOPPS_DB_URL` at a new SQLite file instead of relying on silent repair.

URL pull is opt-in persist: `jobs pull <URL>` defaults to `--save` False; `--no-save` is the ephemeral alias; HTTP cache is independent; persist failure exits 9. Saved rows use reserved `url-pull` digest identity and are not catalog sources. Unscoped `jobs sync` excludes those routes. This page does not authorize Workers upload, Kaggle mutation, hosted-alpha, v7 7.6, or source-policy 1780 grants.
