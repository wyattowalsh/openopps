...
| `url_pull_runs` | Dedicated URL-pull audit domain, distinct from `job_sync_runs`. | Opt-in `jobs pull --save` only. |

Saved URL-pull boards use reserved source key `url-pull` and a punctuation-preserving digest. They are not packaged catalog sources. Default `jobs pull` is ephemeral (`--save` False). Persist failure exits 9. Unscoped `jobs sync` excludes `url-pull` routes. Alembic revisions are `0005_update_snapshot_ledger` then `0006_url_pull_runs`.
...
`boards export` and `jobs export` provide filtered extracts of normalized records. Job JSON/JSONL identifies `profile` and `schemaVersion` (`core`, `search`, `full`, `raw`). CLI default is `full`; generated web search artifacts use `search`. This page does not authorize Workers upload, Kaggle mutation, or a live hosted job database.
...
uv run openopps jobs export --profile search --format jsonl --output /tmp/openopps-jobs.jsonl
...
| Others | See `openspec/changes/archive/2026-07-10-ingest-data-surplus/provider-promotion-manifest.json` |
