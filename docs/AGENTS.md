...
- Keep the static dataset explorer index in `public/data/openopps-search/`; it is generated from `../kaggle/openoppsdb.sqlite` by `scripts/generate_docs_search_index.py` (`pnpm data:generate:search` / `just docs-search-index`) and intentionally does not run during normal docs prebuilds because the SQLite file is local and ignored by git. Committed artifacts use tiered detail shards (T1 metadata, T2 bounded plain-text body ≤4000 chars); never commit `payloadSnapshots`. Release gate: `just docs-search-index-check`. Surplus taxonomy S1–S4: `../openspec/changes/ingest-data-surplus/`.
