...
- Keep the static dataset explorer index in `public/data/openopps-search/`; it is generated from `../kaggle/openoppsdb.sqlite` by `pnpm data:generate:search` and intentionally does not run during normal docs prebuilds because the SQLite file is local and ignored by git.
