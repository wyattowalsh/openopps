...
| Source family                 | Packaged keys         | Default  | Notes                                                                                                    |
| ----------------------------- | --------------------- | -------- | -------------------------------------------------------------------------------------------------------- |
| Official public-company index | `sec-company-tickers` | Disabled | Opt-in listed-company backbone; SEC fair-access controls can reject generic scheduled sync environments. |
| Public index CSV              | `sp500`, `nasdaq100`  | Disabled | Opt-in seed data for index membership; `nasdaq100` is manual until a reviewed CSV is configured.         |
| Employer ranking CSV          | `fortune500`          | Disabled | Opt-in ranking seed data; supports embedded user-supplied CSV rows for reviewed local refreshes.         |
| Ecosystem landscape           | `cncf-landscape`      | Enabled  | Reads CNCF `landscape.yml` public fields and intentionally excludes logos and Crunchbase-derived fields. |
...
Workable, Teamtailor, BambooHR, Rippling, and WP Job Manager are job-capable in v0.1 only through public unauthenticated board routes. BambooHR uses public careers endpoints such as `/careers/list` and `/careers/{job_id}/detail`; OpenOpps does not call authenticated BambooHR ATS APIs. WP Job Manager requires an explicit `/wp-json/wp/v2/job-listings` or `/jm-ajax/get_listings/` endpoint and is not inferred from arbitrary WordPress sites.
