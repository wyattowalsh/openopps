...
The generated Kaggle upload root lives in `kaggle/`. It contains `dataset-metadata.json`, `datapackage.json`, `kernel-metadata.json`, the connected `openoppsdb-manager.ipynb` notebook, and generated SQLite/CSV/Parquet artifacts when a data DB is supplied. Full snapshot bundles also include `sync_metrics.json`, `status.json`, `coverage.json`, and `snapshot-quality.json`. Kaggle may reserve the root `datapackage.json` filename internally, so the generator also publishes the same data dictionary as `metadata/datapackage.json` for live dataset downloads.
...
Use `just kaggle-dataset-create` only for the first public upload; use `just kaggle-dataset-version` after the dataset exists. The live write recipes wrap the Kaggle CLI with `KAGGLE_API_TOKEN` from `kaggle auth print-access-token`, so run `kaggle auth login` before publishing and retry with `kaggle auth login --force` if Kaggle write endpoints return `401`. They fail clearly when OAuth credentials are unavailable and are intentionally outside CI.
