...
The generated Kaggle bundle lives in `kaggle/`. It contains `dataset-metadata.json`, `kernel-metadata.json`, the connected `openoppsdb-manager.ipynb` notebook, and generated SQLite/CSV/Parquet artifacts when a data DB is supplied. `dataset-metadata.json` is the Kaggle UI source of truth for public file descriptions and field-level descriptors for every CSV and Parquet export. Full snapshot runs write private `sync_metrics.json`, `status.json`, `coverage.json`, and `snapshot-quality.json` evidence while validating the bundle, then prune those files before publishing. The live dataset recipes stage a temporary upload directory containing only Kaggle dataset control files plus `openoppsdb.sqlite`, `exports/csv/*.csv`, and `exports/parquet/*.parquet`.
...
The manager notebook should be scheduled in Kaggle as one daily full snapshot, for example `0 6 * * *`. Each run installs OpenOpps from `git+https://github.com/wyattowalsh/openopps.git@main` by default, copies the newest `/kaggle/input/**/openoppsdb.sqlite` ledger into `/kaggle/working/openoppsdb/openoppsdb.sqlite`, runs the unfiltered `openopps sync --metrics-json` workflow, writes private status and coverage evidence, regenerates metadata, exports every SQLite table to CSV and Parquet, writes a private `snapshot-quality.json`, prunes private evidence from the upload directory, and versions the dataset only when the quality gate passes. The notebook seeds bounded Kaggle runtime defaults for source freshness, concurrency, connection limits, timeouts, and retries while allowing `OPENOPPS_` environment overrides.
...
Use `just kaggle-dataset-create` only for the first public upload; use `just kaggle-dataset-version` after the dataset exists. The live write recipes stage a data-only upload directory and wrap the Kaggle CLI with `KAGGLE_API_TOKEN` from `kaggle auth print-access-token`, so run `kaggle auth login` before publishing and retry with `kaggle auth login --force` if Kaggle write endpoints return `401`. They fail clearly when OAuth credentials are unavailable and are intentionally outside CI.
`just kaggle-notebook-push` also passes a one-hour Kaggle kernel timeout by default; override it with `just kaggle-notebook-push timeout=5400` only for a deliberate longer maintenance run.
