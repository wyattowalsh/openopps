...
The docs app exposes three v1 URL surfaces. There is no legacy `/jobs` list route.
...
Bare `/jobs` returns 404. Job detail routes and `/jobs/sitemap/*.xml` remain available under the `/jobs/` prefix.
...
The docs app telemetry source of truth should be a first-party event lake. The default local mode is no-op; production-like deployments should write raw append-only events to persistent storage, compact them into Parquet, and query them with DuckDB or another local analytical engine.

| Sink | Operating model | Notes |
| ---- | --------------- | ----- |
| `noop` | Collects nothing. | Default for local development or hosts without storage. |
| `local-event-lake` | Appends raw NDJSON events, rotates files, and compacts to Parquet. | Canonical free sink when persistent disk is available. |
| `cloudflare` | Optional adapter for Workers/R2/D1/Analytics Engine. | Can be free inside quotas; monitor usage and billing risk. |
| `umami` | Optional self-hosted dashboard mirror. | Useful for summary views, not the raw telemetry warehouse. |

Telemetry operations should preserve the raw event lake, cap payload sizes, record redaction/truncation evidence, and avoid raw IP retention. Keep event names and property keys allowlisted so exploratory UI instrumentation does not accidentally persist secrets from query strings, stack traces, or copied provider payloads.
...
| `OPENOPPS_TELEMETRY_SALT` | test/dev fallback | Required for `local-event-lake`; rotate through deployment config, not source. |
