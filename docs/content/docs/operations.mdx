...
Use this page for local operations and docs validation while working on OpenOpps.
...
The CLI route-probing command and helper script share the same dry-run-first behavior:
...
`pnpm types:check` regenerates Fumadocs MDX artifacts, generates Next.js route types, and runs TypeScript without emitting files.
