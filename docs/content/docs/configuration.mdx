...
Docs-app telemetry is configured separately from CLI runtime settings. When instrumentation is enabled, keep the sink first-party and env-gated: default to a no-op sink in local development, use a persistent local event lake for canonical raw events, and treat Cloudflare or Umami adapters as optional mirrors. See [Data Model](/docs/data-model#telemetry-event-lake) for the event shape and [Operations](/docs/operations#telemetry-operations) for operating guidance.
