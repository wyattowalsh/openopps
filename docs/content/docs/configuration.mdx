...
Docs-app telemetry is configured separately from CLI runtime settings. When instrumentation is enabled, keep the sink first-party and env-gated: default to a no-op sink in local development, use `local-event-lake` for canonical raw events on persistent disk, and treat Cloudflare, Umami, or BI dashboards as downstream mirrors rather than runtime sink names. See [Data Model](/docs/data-model#telemetry-event-lake) for the event shape and [Operations](/docs/operations#telemetry-operations) for operating guidance.
