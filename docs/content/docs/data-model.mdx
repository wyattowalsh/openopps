...
The current `/api/telemetry` route treats any unsupported sink value as `noop`. Cloudflare, Umami, or hosted analytics dashboards can mirror sanitized summaries downstream, but the canonical raw sink remains `local-event-lake`.
