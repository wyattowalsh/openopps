...
```mermaid
flowchart LR
  sources[Sources] --> boards[Boards]
  boards --> routes[Provider routes]
...
`jobs sync` targets every ready job-capable provider route unless narrowed with `--source`, `--board`, or `--provider`. It skips route hints that still lack executable metadata, reports duplicate route skips in metrics, and can also stream synced jobs to JSONL with `--output`.

`jobs list` and `jobs export` share these filters. They use normalized job fields from OpenOpps enrichment, not provider-specific `raw_listing` or `raw_detail` payloads:
...
`examples seed` writes deterministic synthetic sources, boards, routes, jobs, and cache records into the configured local database/cache without hitting upstream services.
...
SQLite is the default storage backend and uses the configured `OPENOPPS_DB_URL`. `admin db init` applies the current v0.1 Alembic schema head for the durable app database. The optional HTTP response cache is separate and is not managed by Alembic.
