...
`sync` runs `sources sync`, `boards sync`, then `jobs sync` with the same source, board, provider, cache, metrics, and verbosity conventions as the individual stages. Human output uses compact stage-aware progress; `--metrics-json` prints one combined metrics payload without progress text on stdout. The `jobs` count is the number of postings fetched from successful provider routes, while `jobsPersisted`, `jobSyncRuns`, and `jobsDeduped` report the unique route observations written to SQLite.
...
`jobs sync` targets every ready job-capable provider route unless narrowed with `--source`, `--board`, or `--provider`. It skips route hints that still lack executable metadata, reports duplicate route skips in metrics, and can also stream synced jobs to JSONL with `--output`. Each successful route sync records a sync run, one observation per unique seen posting, raw payload snapshots, and a normalized job version only when user-visible content changes. When a previously open job is absent from a successful provider response for the same board/provider route, OpenOpps marks it `closed`; provider errors and skipped routes never close jobs. In metrics output, `jobs` remains the fetched-provider count, `jobsPersisted` is the unique observed postings recorded across route runs, `jobSyncRuns` is the count of successful route write transactions, and `jobsDeduped` is the fetched-minus-persisted duplicate count.
