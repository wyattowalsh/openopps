...
OpenOpps is a CLI-only Python app for discovering firm hiring boards from aggregate sources, detecting public provider routes, syncing normalized jobs, caching repeated HTTP JSON requests, and exporting an auditable local opportunity ledger.

## Domain

- `sources` are aggregate discovery catalogs such as `a16z`, `accel`, `generalcatalyst`, `lsvp`, `sequoia`, `bvp`, `greylock`, `kleinerperkins`, and `yc`.
...
List, export, and sync commands default to the full known superset unless narrowed by `--source`, `--board`, or `--provider`. Provider filters accept `any` and `all` as aliases for the full supported provider set.

Overlapping source coverage is preserved with source-scoped board keys such as `a16z:acme`, but provider requests are deduped before route probing or job sync. The relevant metrics include `duplicateRoutesSkipped`.
