...
`pnpm data:generate:search` refreshes the committed static explorer snapshot from `../kaggle/openoppsdb.sqlite`; it is explicit because that SQLite file is local and ignored by git.
...
| `public/data/openopps-search/`  | Static board, provider, and latest-job index generated by `pnpm data:generate:search`. |
