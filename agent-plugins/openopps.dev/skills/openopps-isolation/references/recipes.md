...
Acceptance is only `openopps.discovery.isolation.launch_isolated_scout`.
Calling `validate_data_only_suggestion` directly is not acceptance.

Committed fixtures:

```bash
uv run python agent-plugins/openopps.dev/skills/openopps-source-scout/scripts/validate_fixture.py --help
```

The worker is credential-free and writes only a parent-owned new quarantine file.
That is an application contract, not an OS sandbox.
...
uv run python scripts/audit.py skills/openopps-isolation/
uv run python scripts/package.py skills/openopps-isolation/ --dry-run
```
