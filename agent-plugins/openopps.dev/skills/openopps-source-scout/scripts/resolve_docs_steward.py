...
def _repository_root(start: Path) -> Path:
    ...
    for candidate in (current, *current.parents):
        ...
        try:
            ...
            continue
        if 'name = "openopps"' in text or "name = 'openopps'" in text:
            return candidate
    raise FileNotFoundError(
        f"OpenOpps pyproject.toml not found walking from {start}"
    )
...
RESOLVER = "agent-plugins/openopps.dev/skills/openopps-source-scout/scripts/resolve_docs_steward.py"
SEARCH_TIMEOUT_S = 30
