# Contributor hub recipes

Hand off; do not run public sync here.

- scout skill: `openopps-source-scout`
- CLI: `openopps-discovery`
- isolation: `openopps-isolation`
- evals: `openopps-discovery-evals`

MCP `run` allows only `discovery scout|verify-scout|preview-promotion` (JSON).
Keep `.agents/skills/openopps-source-scout/` and `.cursor/skills/openopps-source-scout/` absent.
...
uv run python scripts/audit.py skills/openopps-dev/
uv run python scripts/package.py skills/openopps-dev/ --dry-run
```
