# Discovery eval recipes

```bash
OPENOPPS_DISCOVERY_NETWORK=disabled uv run python agent-plugins/openopps.dev/skills/openopps-source-scout/scripts/validate_evals.py
OPENOPPS_DISCOVERY_NETWORK=disabled uv run python agent-plugins/openopps.dev/skills/openopps-source-scout/scripts/validate_frontmatter.py
OPENOPPS_DISCOVERY_NETWORK=disabled uv run python agent-plugins/openopps.dev/skills/openopps-source-scout/scripts/dry_run_projection.py
OPENOPPS_DISCOVERY_NETWORK=disabled uv run python agent-plugins/openopps.dev/skills/openopps-source-scout/scripts/resolve_docs_steward.py
OPENOPPS_DISCOVERY_NETWORK=disabled just source-discovery-skill-eval-check
OPENOPPS_DISCOVERY_NETWORK=disabled just ci-discovery
```

These stay `uv run python` / `just` commands, not MCP tools.
...
uv run python scripts/audit.py skills/openopps-discovery-evals/
uv run python scripts/package.py skills/openopps-discovery-evals/ --dry-run
```
