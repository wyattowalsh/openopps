...
openopps examples seed --help
```

`plugins list` is Python `openopps.plugins`. Do not purge cache here unless handed to `openopps-admin`.
...
uv run python scripts/audit.py skills/openopps-operations/
uv run python scripts/package.py skills/openopps-operations/ --dry-run
```
