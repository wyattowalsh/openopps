...
```bash
openopps admin db status --json
openopps admin db init
openopps admin cache purge --help
openopps admin sources list --json
openopps admin providers probe-routes --help
```

Refuse `admin sources scout|verify-scout|preview-promotion`.
Probe-routes persist only with `--apply`.
...
uv run python scripts/audit.py skills/openopps-admin/
uv run python scripts/package.py skills/openopps-admin/ --dry-run
```
