...
```bash
openopps jobs list --json
openopps jobs list --source example --json
openopps jobs show <job-id>
openopps jobs history <job-id>
openopps jobs export --output jobs.jsonl --json
```
...
uv run python scripts/audit.py skills/openopps-jobs/
uv run python scripts/package.py skills/openopps-jobs/ --dry-run
```
