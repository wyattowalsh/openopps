...
```bash
openopps boards list --json
openopps boards show <board-id> --json
openopps boards sync --json
openopps boards export --output boards.jsonl
```

Board keys are `source:slug`. Public `boards` is not `admin boards add`.
...
uv run python scripts/audit.py skills/openopps-boards/
uv run python scripts/package.py skills/openopps-boards/ --dry-run
```
