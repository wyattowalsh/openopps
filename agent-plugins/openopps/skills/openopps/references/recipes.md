...
Resolve the CLI, then prefer JSON.

```bash
openopps --help
openopps status --json
openopps doctor --json
```
...
OPENOPPS_BIN=$(command -v openopps)
```

Local DB:

```bash
OPENOPPS_DB_URL=sqlite:///./tmp/openopps.sqlite openopps admin db init
openopps plugins list --json
openopps cache stats --json
```

Do not run `openopps discovery ...` from this plugin.
...
uv run python scripts/audit.py skills/openopps/
uv run python scripts/package.py skills/openopps/ --dry-run
```
