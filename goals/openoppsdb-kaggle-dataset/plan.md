...
Treat `scripts/generate_kaggle_metadata.py` as the source of truth for the Kaggle upload root and manager notebook. Extend it so the generated notebook performs the daily full sync, records run evidence, validates snapshot quality before publishing, exports every table, and versions the Kaggle dataset only after hard gates pass.
...
2. Add or update OpenSpec requirements for the Kaggle dataset workflow.
   ...
   - Add requirements for a daily full OpenOppsDB dataset run, generated SQLite/CSV/Parquet/data-dictionary artifacts, quality-gated publish behavior, provider-error classification, and live post-deploy verification.
...
4. Capture run evidence during the manager run.
   - Touches: `scripts/generate_kaggle_metadata.py`, generated `kaggle/dataset-metadata.json`, `kaggle/datapackage.json`, generated notebook.
   - Capture and publish `sync_metrics.json` from `openopps sync --metrics-json`.
   - Capture and publish `status.json` from `openopps status --json`.
   - Consider publishing `coverage.json` from `openopps providers coverage --json` if the local command remains persisted-data-only and useful for dataset users.
   - Add concise resource descriptions for every new evidence file.
   - Verification: tests assert new evidence files are declared in dataset metadata and datapackage metadata, and the notebook writes them before publishing.
...
5. Implement snapshot quality gates before publishing.
   ...
   - Write a `snapshot-quality.json` report that records pass/fail status, hard blockers, warnings, counts, metrics excerpts, provider error summaries, required-file checks, and any explicit empty/outage explanation.
...
6. Keep full table export and metadata coverage complete.
   - Touches: `scripts/generate_kaggle_metadata.py`, generated `kaggle/datapackage.json`, generated `kaggle/dataset-metadata.json`, generated SQLite metadata tables.
   ...
   - Ensure any new evidence artifacts are described as files, while every SQLite table still has CSV/Parquet exports and table/column metadata.
   - Verification: `uv run pytest tests/unit/openopps/test_kaggle_metadata.py -q` plus an export test against a temporary SQLite DB verifying every declared table export exists and every data artifact is described.
...
8. Update generated artifacts and docs.
   - Touches: `kaggle/dataset-metadata.json`, `kaggle/datapackage.json`, `kaggle/kernel-metadata.json`, `kaggle/openoppsdb-manager.ipynb`, `docs/content/docs/operations.mdx`, `README.md`, possibly `docs/lib/generated/openopps-data.json` if docs generation changes it.
...
10. Deploy and verify live Kaggle surfaces.
    ...
    - Download or inspect the live dataset artifacts, then verify SQLite readability, required files, CSV/Parquet exports, metadata files, and `snapshot-quality.json`.
...
- If new evidence files are added, they become public dataset file-format surfaces and must be described, tested, and documented alongside existing artifacts.
