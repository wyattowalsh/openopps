...
METADATA_TABLES: tuple[Table, ...] = (
    Table(
        ...
        description="In-database table labels and descriptions for openopps.sqlite.",
    ),
    Table(
        name="openopps_columns",
        model=OpenOppsColumnRow,
        description="In-database column labels, descriptions, and schema hints for openopps.sqlite.",
    ),
)
...
- `openopps.sqlite`: the complete SQLite ledger, including metadata tables named `openopps_tables` and `openopps_columns`.
...
conn = sqlite3.connect('/kaggle/input/openoppsdb/openopps.sqlite')
