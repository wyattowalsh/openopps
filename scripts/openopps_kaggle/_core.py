    ...
    return {
        "cells": [
            _markdown_cell(
                ...
                "--metrics-json --freshness-seconds --limit`. When every "
                "`sources` row is the synthetic example catalog (`key=example` "
                "or `example://…`), the manager skips `openopps jobs sync` and "
                "republishes the rehydrated snapshot so example routes are not "
                "marked detect-only. It captures status "
