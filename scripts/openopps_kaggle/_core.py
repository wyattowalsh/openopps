...
from openopps_kaggle.constants import (
    ...
    EXPLORER_NB_FILE,
    EXPLORER_NB_ID,
    PUBLIC_KERNEL_KEYWORDS,
    SKILLS_RADAR_NB_FILE,
    SKILLS_RADAR_NB_ID,
    SNAPSHOT_HEALTH_NB_FILE,
    SNAPSHOT_HEALTH_NB_ID,
    SNAPSHOT_QUALITY_FILE,
    SQL_PLAYGROUND_NB_FILE,
    SQL_PLAYGROUND_NB_ID,
    sibling_kernel_sources,
...
def dataset_metadata() -> dict[str, Any]:
    return {
        ...
        "title": "OpenOppsDB",
        "subtitle": (
            "Daily public hiring-board ledger (SQLite, CSV, Parquet) "
            "with SQL and Gradio notebooks"
        ),
        "description": _dataset_description(),
        "licenses": [{"name": DATASET_LICENSE}],
        "keywords": list(DATASET_MARKETPLACE_KEYWORDS),
...
def _dataset_description() -> str:
    return (
        "# OpenOppsDB\n\n"
        "OpenOppsDB is a versioned public hiring-board ledger generated by the "
        "OpenOpps CLI. It tracks discovered company boards, executable provider "
        "routes, normalized job identities, versioned job content, raw provider "
        "payload snapshots, and sync observations over time.\n\n"
        "## What is included\n\n"
        "- `openoppsdb.sqlite`: the OpenOppsDB SQLite database file, including "
        "metadata tables named `openopps_tables` and `openopps_columns`.\n"
        ...
        "- `exports/parquet/*.parquet`: full table exports for Python, DuckDB, "
        "Polars, Spark, and warehouse workflows, including rendered HTML "
        "descriptions, structured job-description JSON, and raw payloads.\n\n"
        "## Notebooks\n\n"
        "Start with **Starter**, then open the featured **Explorer** Gradio app. "
        "All public kernels are read-only against this dataset.\n\n"
        f"{_notebook_index_markdown()}\n\n"
        "## Quick start\n\n"
        "```python\n"
        "import sqlite3\n"
        "import polars as pl\n\n"
        ...
        "'/kaggle/input/openoppsdb/exports/parquet/job_versions.parquet')\n"
        "```\n\n"
        "## Notes and limitations\n\n"
        "OpenOpps only uses public endpoints and public pages. Provider payloads "
        "are preserved for auditability, but normalized fields should be treated "
        "as best-effort public-data extraction rather than official ATS records. "
        "Kaggle renders CSV and Parquet exports as the primary tabular preview "
        "and column-metadata surfaces; if Kaggle does not expose nested SQLite "
        "table previews for a fresh upload, open the SQLite file directly or use "
        "the mirrored CSV/Parquet exports. A row can appear across multiple "
        "source catalogs; durable keys and sync observation tables are provided "
        "so downstream users can reason about provenance and change history.\n\n"
        "The public file surface is intentionally limited to `openoppsdb.sqlite`, "
        "`exports/csv/*.csv`, and `exports/parquet/*.parquet`. Public publish "
        "quality gates hard-block oversize artifacts (SQLite above 2 GiB or "
        "combined CSV/Parquet exports above 4 GiB). Local maintainer "
        "create/version recipes rebuild from an explicit clean-schema DB path "
        "before staging; do not upload legacy ledgers that still carry removed "
        "columns such as `sources.enabled`.\n"
    )
...
def kernel_metadata() -> dict[str, Any]:
    return {
        ...
        "keywords": list(PUBLIC_KERNEL_KEYWORDS),
...
def public_notebook_kernel_metadata(
    *,
    ...
    kernel_sources: tuple[str, ...] | list[str] = (),
) -> dict[str, Any]:
    return {
        "id": notebook_id,
        "title": title,
...
def kernel_metadata_for_spec(spec: PublicNotebookSpec) -> dict[str, Any]:
    return public_notebook_kernel_metadata(
        ...
        kernel_sources=spec.kernel_sources,
    )
...
def starter_kernel_metadata() -> dict[str, Any]:
    return public_notebook_kernel_metadata(
        ...
        kernel_sources=sibling_kernel_sources(STARTER_NB_ID),
    )
    ...
    return {
        "cells": [
            _markdown_cell(
                "overview",
                "# OpenOppsDB — Manager\n\n"
                "Private scheduled notebook connected to `wyattowalsh/openoppsdb`.\n\n"
                "## Status\n\n"
                "1. Setup and install OpenOpps from the pinned git SHA.\n"
                "2. Sync public catalogs, boards, and jobs under the 6000s budget.\n"
                "3. Export SQLite/CSV/Parquet and run the fail-closed quality gate.\n"
                "4. Publish a new dataset version only if that gate passes.\n\n"
                "Schedule daily (for example `0 6 * * *`). This notebook is **not** "
                "a public example. Public readers should use Starter and Explorer "
                "on the dataset page.\n\n"
                "Each run copies the newest `/kaggle/input/**/openoppsdb.sqlite` "
                ...
                "snapshot into an operational Alembic schema when needed, then "
                "runs `openopps sync --metrics-json` (packaged catalog sources, "
                "boards, and jobs). It captures status and coverage evidence for "
                "the private quality gate, then invokes `python -m openopps_kaggle` "
                "to backfill derived skill tables, prepare artifacts, prune private "
                "evidence, stage the public upload directory, deploy a new dataset "
                "version only after the quality gate passes, and attempt best-effort "
                "live Kaggle file metadata repair. Local "
                "`just kaggle-live-file-metadata` runs use browser-authenticated "
                "cookies for the authoritative Kaggle DataBundle checklist repair.",
            ),
...
def _notebook_index_markdown() -> str:
    from openopps_kaggle.notebooks.studio import notebook_index_markdown

    return notebook_index_markdown()


def starter_notebook() -> dict[str, Any]:
    from openopps_kaggle.notebooks.studio import starter_notebook as _studio

    return _studio()


def advanced_usage_notebook() -> dict[str, Any]:
    from openopps_kaggle.notebooks.studio import advanced_usage_notebook as _studio

    return _studio()


def hiring_market_map_notebook() -> dict[str, Any]:
    from openopps_kaggle.notebooks.studio import hiring_market_map_notebook as _studio

    return _studio()


def skills_radar_notebook() -> dict[str, Any]:
    from openopps_kaggle.notebooks.studio import skills_radar_notebook as _studio

    return _studio()


def sql_playground_notebook() -> dict[str, Any]:
    from openopps_kaggle.notebooks.studio import sql_playground_notebook as _studio

    return _studio()


def explorer_notebook() -> dict[str, Any]:
    from openopps_kaggle.notebooks.studio import explorer_notebook as _studio

    return _studio()


def snapshot_health_notebook() -> dict[str, Any]:
    from openopps_kaggle.notebooks.studio import snapshot_health_notebook as _studio
...
PUBLIC_EXAMPLE_NOTEBOOKS: tuple[PublicNotebookSpec, ...] = (
    PublicNotebookSpec(
        ...
        enable_internet=True,
        keywords=PUBLIC_KERNEL_KEYWORDS,
        kernel_sources=sibling_kernel_sources(ADVANCED_NB_ID),
    ),
    PublicNotebookSpec(
        ...
        keywords=PUBLIC_KERNEL_KEYWORDS,
        kernel_sources=sibling_kernel_sources(HIRING_MARKET_NB_ID),
    ),
    PublicNotebookSpec(
        slug="skills-radar",
        ...
        enable_internet=False,
        keywords=PUBLIC_KERNEL_KEYWORDS,
        kernel_sources=sibling_kernel_sources(SKILLS_RADAR_NB_ID),
    ),
    PublicNotebookSpec(
        slug="sql-playground",
        notebook_id=SQL_PLAYGROUND_NB_ID,
        title="OpenOppsDB — SQL playground",
        code_file=SQL_PLAYGROUND_NB_FILE,
        notebook_factory=sql_playground_notebook,
        enable_internet=True,
        keywords=PUBLIC_KERNEL_KEYWORDS,
        kernel_sources=sibling_kernel_sources(SQL_PLAYGROUND_NB_ID),
    ),
    PublicNotebookSpec(
        slug="explorer",
        notebook_id=EXPLORER_NB_ID,
        title="OpenOppsDB — Explorer",
        code_file=EXPLORER_NB_FILE,
        notebook_factory=explorer_notebook,
        enable_internet=True,
        keywords=PUBLIC_KERNEL_KEYWORDS,
        kernel_sources=sibling_kernel_sources(EXPLORER_NB_ID),
    ),
    PublicNotebookSpec(
        slug="snapshot-health",
        notebook_id=SNAPSHOT_HEALTH_NB_ID,
        title="OpenOppsDB — Snapshot health",
        code_file=SNAPSHOT_HEALTH_NB_FILE,
        notebook_factory=snapshot_health_notebook,
        enable_internet=False,
        keywords=PUBLIC_KERNEL_KEYWORDS,
        kernel_sources=sibling_kernel_sources(SNAPSHOT_HEALTH_NB_ID),
    ),
)
...
def _drop_private_sqlite_tables(db_path: Path) -> None:
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        for name in names:
            if name.startswith("sqlite_") or name in expected:
                continue
            conn.execute(f"DROP TABLE IF EXISTS {_sqlite_identifier(name)}")
        conn.commit()
...
openopps_env.setdefault("OPENOPPS_CACHE_ENABLED", "true")
