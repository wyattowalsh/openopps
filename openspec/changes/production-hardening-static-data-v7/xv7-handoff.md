# XV701–XV799 shared-surface handoff (evidence note)

Copied 2026-08-22 from
`openspec/changes/bounded-quarantined-source-discovery/xv7-handoff.md`
into this v7 change as **documentation only**.

This note does **not** close live Workers tasks 5.4–5.7, v6 dual-read 2.10,
or ordinary-removal / rewrite-prep 7.1–7.7. `W-SHARED-DELIVERY` stays
inactive. Only `src/openopps/ingest.py`, `src/openopps/storage.py`,
`src/openopps/models.py`, and `src/openopps/alembic/**` were handed to
`W-STORAGE` / `W-INGEST`. Discovery continues to read-and-hash remaining
v7 surfaces.
