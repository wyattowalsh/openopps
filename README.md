...
Publication is fail-closed on source rights and required attribution. `just source-policy-check` validates the canonical evidence, schema, and exact committed-v6 corpus identity; it is a structural CI gate, not permission to publish. `just source-policy-audit` is the release-eligibility gate. The current audit is deliberately red: 7 of 1787 sources only mirror repository catalog declarations, 0 are independently verified, and 1780 are blocked. Do not render a selector, generate or upload a production corpus, bootstrap a Worker, or publish while that audit exits 2. The generator applies the evidence as a deny-only overlay and hashes its module, evidence, schema, and corpus into the v7 release identity, so catalog or stored metadata cannot override a reviewed denial.
...
# Bounded local seed for a maintainer ledger. The manager notebook instead runs
# the full `openopps sync --metrics-json` pipeline under its notebook time budget.
